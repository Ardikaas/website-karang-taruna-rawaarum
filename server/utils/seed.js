...
const seedDatabase = async () => {
  try {
      ...
      const seedItems = [
        {
          ...
          description:
            'Informasi lowongan kerja terbaru, pelatihan kerja, dan program magang untuk warga Rawaarum.',
          type: 'loker',
          date: '10 Juli 2026',
        ...
        },
        {
          title: 'Potensi UMKM',
          description:
            'Kenali produk unggulan kreatif, kerajinan tangan, dan usaha kuliner warga binaan Rawaarum.',
          type: 'umkm',
          date: '08 Juli 2026',
        ...
        },
        {
          title: 'Info Kegiatan',
          description:
            'Update agenda, dokumentasi gotong royong, dan kegiatan sosial kepemudaan Karang Taruna.',
          type: 'kegiatan',
          date: '05 Juli 2026',
        ...
        },
        {
          title: 'Pengumuman',
          description:
            'Informasi penting kelurahan, rilis kebijakan pemuda, serta agenda rapat umum bulanan warga.',
          type: 'pengumuman',
          date: '01 Juli 2026',
    ...
    // Ensure Pengurus Account (Ardika Aji Setiawan) exists
    const existingArdika = await User.findOne({
      $or: [{ username: 'ardika aji setiawan' }, { username: 'ardika' }],
    });
    if (!existingArdika) {
      const salt = await bcrypt.genSalt(10);
      const pengurusPassHash = await bcrypt.hash('Password123', salt);
      await User.create({
        username: 'ardika aji setiawan',
        passwordHash: pengurusPassHash,
        name: 'Ardika Aji Setiawan',
        role: 'pengurus',
      });
      console.log(
        'Created Pengurus Account: Username "Ardika Aji Setiawan" | Password "Password123"'
      );
    }
    ...
    if (pengurusCount === 0) {
      ...
      const bidangRaw = [
        ...
        },
        {
          ...
          anggota: [
            'Yuda Andika',
            'Dede Sapta Kurniawan',
            'Muhlis',
            'Saiful Anwar',
            'Agus Hairil Anam',
            'Muhammad Farhan Dzulqisthi',
          ],
        },
        {
          id: 'ekonomi',
